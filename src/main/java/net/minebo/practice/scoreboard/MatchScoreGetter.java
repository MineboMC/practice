...
import net.minebo.practice.util.scoreboard.construct.ScoreFunction;
...
final class MatchScoreGetter implements BiConsumer<Player, List<String>> {
    ...
    public void accept(Player player, List<String> scores) {
        ...
        if (match == null) {
            followingOpt.ifPresent(uuid -> scores.add("&fFollowing: &e" + Practice.getInstance().getUuidCache().name(uuid)));
    ...
    private void render1v1MatchLines(List<String> scores, MatchTeam otherTeam) {
        scores.add("&fOpponent: &e" + Bukkit.getPlayer(otherTeam.getFirstMember()).getDisplayName());
    ...
    private void render2v2MatchLines(List<String> scores, MatchTeam ourTeam, MatchTeam otherTeam, Player player, HealingMethod healingMethod) {
        ...
        if (partnerUuid != null) {
            ...
            scores.add(namePrefix + Practice.getInstance().getUuidCache().name(partnerUuid));
    ...
    private void renderSpectatorLines(List<String> scores, Match match, MatchTeam oldTeam) {
        ...
        if (teams.size() == 2) {
            ...
            if (teamOne.getAllMembers().size() != 1 && teamTwo.getAllMembers().size() != 1) {
                ...
                if (oldTeam == null) {
                    scores.add("&fTeam A: &e" + teamOne.getAliveMembers().size() + "/" + teamOne.getAllMembers().size());
                    scores.add("&fTeam B: &e" + teamTwo.getAliveMembers().size() + "/" + teamTwo.getAllMembers().size());
                } else {
                    MatchTeam otherTeam = oldTeam == teamOne ? teamTwo : teamOne;

                    scores.add("&fTeam: &e" + oldTeam.getAliveMembers().size() + "/" + oldTeam.getAllMembers().size());
                    scores.add("&fOpponents: &e" + otherTeam.getAliveMembers().size() + "/" + otherTeam.getAllMembers().size());
                }
            }
        }
    ...
    private void renderMetaLines(List<String> scores, Match match, boolean participant) {
        ...
        scores.add("&fDuration: &e" + formattedDuration);
    }
    ...
    private List<String> renderTeamMemberOverviewLinesWithHearts(MatchTeam team) {
        ...
        for (UUID teamMember : team.getAllMembers()) {
            if (team.isAlive(teamMember)) {
                aliveLines.add(" &f" + Practice.getInstance().getUuidCache().name(teamMember) + " " + getHeartString(team, teamMember));
            } else {
                deadLines.add(" &7&m" + Practice.getInstance().getUuidCache().name(teamMember));
            }
        }
    ...
    private List<String> renderTeamMemberOverviewLines(MatchTeam team) {
        ...
        for (UUID teamMember : team.getAllMembers()) {
            if (team.isAlive(teamMember)) {
                aliveLines.add(" &f" + Practice.getInstance().getUuidCache().name(teamMember));
            } else {
                deadLines.add(" &7&m" + Practice.getInstance().getUuidCache().name(teamMember));
            }
        }
