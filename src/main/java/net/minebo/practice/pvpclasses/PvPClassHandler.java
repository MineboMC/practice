package net.minebo.practice.pvpclasses;
...
public class PvPClassHandler extends BukkitRunnable implements Listener {
    ...
    public PvPClassHandler() {
        ...
        Bukkit.getServer().getScheduler().runTaskTimer(Practice.getInstance(), this, 2L, 2L);
        Bukkit.getServer().getPluginManager().registerEvents(this, Practice.getInstance());
    }

    public void shutdown() {
        for (String playerName : getEquippedKits().keySet()) {
            getEquippedKits().get(playerName).remove(Practice.getInstance().getServer().getPlayerExact(playerName));
        }
    }
